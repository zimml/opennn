...
/// Default constructor.
/// It creates a conjugate gradient optimization algorithm object not associated
/// with any loss index object. It also initializes the class members to their
/// default values.
...
/// It creates a conjugate gradient optimization algorithm associated with a
/// loss index object. It also initializes the rest of the class members to
...
void ConjugateGradient::calculate_conjugate_gradient_training_direction(
	...
	Tensor<type, 1>& training_direction) const
{
	...
	if (old_training_direction_size != parameters_number)
	{
		buffer << "OpenNN Exception: ConjugateGradient class.\n"
			   << "void calculate_training_direction() const method.\n"
			   << "Size of old training direction ("
			   << old_training_direction_size
			   << ") is not equal to number of parameters ("
			   << parameters_number << ").\n";
	...
	case TrainingDirectionMethod::FR:
		calculate_FR_training_direction(old_gradient,
										gradient,
	...
	case TrainingDirectionMethod::PR:
		calculate_PR_training_direction(old_gradient,
										gradient,
										old_training_direction,
										training_direction);
		return;
...
type ConjugateGradient::calculate_FR_parameter(
	const Tensor<type, 1>& old_gradient, const Tensor<type, 1>& gradient) const
{
#ifdef OPENNN_DEBUG
	...
	{
		buffer << "OpenNN Exception: ConjugateGradient class.\n"
			   << "type calculate_FR_parameter(const Tensor<type, 1>&, const "
				  "Tensor<type, 1>&) const method.\n"
	...
	denominator.device(*thread_pool_device) =
		old_gradient.contract(old_gradient, AT_B);
...
void ConjugateGradient::calculate_FR_training_direction(
	const Tensor<type, 1>& old_gradient,
	const Tensor<type, 1>& gradient,
	const Tensor<type, 1>& old_training_direction,
	Tensor<type, 1>& training_direction) const
{
	...
	training_direction.device(*thread_pool_device) =
		-gradient + old_training_direction * FR_parameter;
}

/// Returns the dradient descent training direction.
/// @param gradient Current error function gradient.

void ConjugateGradient::calculate_gradient_descent_training_direction(
	const Tensor<type, 1>& gradient, Tensor<type, 1>& training_direction) const
{
...
type ConjugateGradient::calculate_PR_parameter(
	const Tensor<type, 1>& old_gradient, const Tensor<type, 1>& gradient) const
{
#ifdef OPENNN_DEBUG
	...
	{
		buffer << "OpenNN Exception: ConjugateGradient class.\n"
			   << "type calculate_PR_parameter(const Tensor<type, 1>&, const "
				  "Tensor<type, 1>&) const method.\n"
			   << "Loss index pointer is nullptr.\n";
	...
	numerator.device(*thread_pool_device) =
		(gradient - old_gradient).contract(gradient, AT_B);
	denominator.device(*thread_pool_device) =
		old_gradient.contract(old_gradient, AT_B);
...
void ConjugateGradient::calculate_PR_training_direction(
	const Tensor<type, 1>& old_gradient,
	const Tensor<type, 1>& gradient,
	const Tensor<type, 1>& old_training_direction,
	Tensor<type, 1>& training_direction) const
{
	...
	training_direction.device(*thread_pool_device) =
		-gradient + old_training_direction * PR_parameter;
}

/// Returns a constant reference to the learning rate algorithm object inside
/// the conjugate gradient method object.

const LearningRateAlgorithm& ConjugateGradient::get_learning_rate_algorithm()
	const
{
	return learning_rate_algorithm;
}

/// Returns a pointer to the learning rate algorithm object inside the conjugate
...
ConjugateGradient::get_training_direction_method() const
{
...
/// Sets a pointer to a loss index object to be associated with the conjugate
/// gradient object. It also sets that loss index to the learning rate
/// algorithm.
/// @param new_loss_index_pointer Pointer to a loss index object.

void ConjugateGradient::set_loss_index_pointer(
...
/// representation. Possible values are: <ul> <li> "PR" <li> "FR"
/// </ul>
/// @param new_training_direction_method_name String with the name of the
/// training direction method.

void ConjugateGradient::set_training_direction_method(
	const string& new_training_direction_method_name)
{
...
void ConjugateGradient::set_maximum_selection_failures(
...
void ConjugateGradient::set_maximum_epochs_number(
...
void ConjugateGradient::set_minimum_loss_decrease(
...
/// conjugate gradient algorithm. Training occurs according to the training
/// operators, training parameters and stopping criteria.
	...
	ConjugateGradientData& optimization_data) const
{
	...
	if (optimization_data.epoch == 0 ||
		optimization_data.epoch % parameters_number == 0)
	{
	...
	forward_propagation.neural_network_pointer->set_parameters(
		back_propagation.parameters);
}
...
/// Serializes the conjugate gradient object into an XML document of the TinyXML
/// library without keeping the DOM tree in memory. See the OpenNN manual for
/// more information about the format of this element.
...
/// Deserializes the conjugate gradient object from an XML document of the
/// TinyXML library.
...
void ConjugateGradient::from_XML(const tinyxml2::XMLDocument& document)
{
	...
	{
		const string new_training_direction_method =
	...
	if (learning_rate_algorithm_element)
	{
		...
		element_clone = learning_rate_algorithm_element->DeepClone(
			&learning_rate_algorithm_document);
	...
	const tinyxml2::XMLElement* minimum_loss_decrease_element =
		root_element->FirstChildElement("MinimumLossDecrease");

	if (minimum_loss_decrease_element)
	{
		const type new_minimum_loss_decrease =
			static_cast<type>(atof(minimum_loss_decrease_element->GetText()));
	...
	const tinyxml2::XMLElement* loss_goal_element =
		root_element->FirstChildElement("LossGoal");

	if (loss_goal_element)
	{
		const type new_loss_goal =
			static_cast<type>(atof(loss_goal_element->GetText()));
	...
	{
		const Index new_maximum_selection_failures = static_cast<Index>(
	...
	{
		const Index new_maximum_iterations_number = static_cast<Index>(
	...
	if (maximum_time_element)
	{
		const type new_maximum_time =
			static_cast<type>(atof(maximum_time_element->GetText()));
	...
	{
		const Index new_display_period =
			static_cast<Index>(atoi(display_period_element->GetText()));
	...
	const tinyxml2::XMLElement* element =
		root_element->FirstChildElement("SavePeriod");

	if (element)
	{
		const Index new_save_period =
...
void ConjugateGradientData::set(
	ConjugateGradient* new_conjugate_gradient_pointer)
{
	conjugate_gradient_pointer = new_conjugate_gradient_pointer;

	const LossIndex* loss_index_pointer =
		conjugate_gradient_pointer->get_loss_index_pointer();
