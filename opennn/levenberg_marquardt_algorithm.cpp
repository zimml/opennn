...
/// Default constructor.
/// It creates a Levenberg-Marquardt optimization algorithm object not
/// associated with any loss index object. It also initializes the class members
...
/// It creates a Levenberg-Marquardt optimization algorithm object associated
/// associated with a given loss index object. It also initializes the class
...
/// Returns the maximum number of selection failures during the training
/// process.
...
void LevenbergMarquardtAlgorithm::set_damping_parameter(
	const type& new_damping_parameter)
{
...
void LevenbergMarquardtAlgorithm::set_damping_parameter_factor(
	const type& new_damping_parameter_factor)
{
#ifdef OPENNN_DEBUG
	...
	if (new_damping_parameter_factor <= static_cast<type>(0.0))
	{
		...
		buffer << "OpenNN Exception: LevenbergMarquardtAlgorithm class." << endl
			   << "void set_damping_parameter_factor(const type&) method."
			   << endl
...
void LevenbergMarquardtAlgorithm::set_minimum_damping_parameter(
	const type& new_minimum_damping_parameter)
{
#ifdef OPENNN_DEBUG
	...
	if (new_minimum_damping_parameter <= static_cast<type>(0.0))
	{
		...
		buffer << "OpenNN Exception: LevenbergMarquardtAlgorithm class." << endl
			   << "void set_minimum_damping_parameter(const type&) method."
			   << endl
			   << "Minimum damping parameter must be greater than zero."
			   << endl;
...
void LevenbergMarquardtAlgorithm::set_maximum_damping_parameter(
	const type& new_maximum_damping_parameter)
{
#ifdef OPENNN_DEBUG
	...
	if (new_maximum_damping_parameter <= static_cast<type>(0.0))
	{
		...
		buffer << "OpenNN Exception: LevenbergMarquardtAlgorithm class." << endl
			   << "void set_maximum_damping_parameter(const type&) method."
			   << endl
			   << "Maximum damping parameter must be greater than zero."
			   << endl;
...
void LevenbergMarquardtAlgorithm::set_minimum_loss_decrease(
...
void LevenbergMarquardtAlgorithm::set_maximum_epochs_number(
...
/// <ul>
/// <li> The loss index pointer associated with the optimization algorithm is
/// not nullptr, <li> The neural network associated with that loss index is
/// neither nullptr. <li> The data set associated with that loss index is
/// neither nullptr.
/// </ul>
...
void LevenbergMarquardtAlgorithm::check() const
{
	...
	const DataSet* data_set_pointer =
		loss_index_pointer->get_data_set_pointer();
...
/// Levenberg-Marquardt algorithm. Training occurs according to the training
/// parameters.
...

{
	...
	{
		throw invalid_argument(
			"Levenberg-Marquard algorithm cannot work with cross-entropy "
			"error.");
	}
	else if (loss_index_pointer->get_error_type() == "WEIGHTED_SQUARED_ERROR")
	{
		throw invalid_argument(
			"Levenberg-Marquard algorithm is not implemented yet with weighted "
			"squared error.");
	}
	...
	LossIndexBackPropagationLM training_back_propagation_lm(
		training_samples_number,
		loss_index_pointer);
	LossIndexBackPropagationLM selection_back_propagation_lm(
		selection_samples_number,
		loss_index_pointer);
	...
	for (Index epoch = 0; epoch <= maximum_epochs_number; epoch++)
	{
		...
		if (has_selection)
		{
			...
			loss_index_pointer->calculate_errors_lm(
				selection_batch,
				selection_forward_propagation,
				selection_back_propagation_lm);

			loss_index_pointer->calculate_squared_errors_lm(
				selection_batch,
				selection_forward_propagation,
				selection_back_propagation_lm);

			loss_index_pointer->calculate_error_lm(
				selection_batch,
				selection_forward_propagation,
				selection_back_propagation_lm);

			results.selection_error_history(epoch) =
				selection_back_propagation_lm.error;
		...
		if (display && epoch % display_period == 0)
		{
			cout << "Training error: " << training_back_propagation_lm.error
				 << endl;
			if (has_selection)
				cout << "Selection error: "
					 << selection_back_propagation_lm.error << endl;
		...
		{
			TrainingStep step;
			step.trainingError	= training_back_propagation_lm.error;
			step.selectionError = selection_back_propagation_lm.error;
			step.damping		= damping_parameter;
		...
		if (epoch != 0)
			loss_decrease = old_loss - training_back_propagation_lm.loss;
...
void LevenbergMarquardtAlgorithm::update_parameters(
	const DataSetBatch& batch,
	NeuralNetworkForwardPropagation& forward_propagation,
	LossIndexBackPropagationLM& back_propagation_lm,
	LevenbergMarquardtAlgorithmData& optimization_data)
{
	const type regularization_weight =
		loss_index_pointer->get_regularization_weight();
	...
	do
	{
		...
		optimization_data.parameters_increment =
			perform_Householder_QR_decomposition(
				back_propagation_lm.hessian,
				(type(-1)) * back_propagation_lm.gradient);

		optimization_data.potential_parameters.device(*thread_pool_device) =
			back_propagation_lm.parameters +
			optimization_data.parameters_increment;
		...
		loss_index_pointer->calculate_errors_lm(batch,
												forward_propagation,
												back_propagation_lm);

		loss_index_pointer->calculate_squared_errors_lm(batch,
														forward_propagation,
														back_propagation_lm);

		loss_index_pointer->calculate_error_lm(batch,
											   forward_propagation,
											   back_propagation_lm);
		...
		try
		{
			new_loss = back_propagation_lm.error +
					   regularization_weight *
						   loss_index_pointer->calculate_regularization(
							   optimization_data.potential_parameters);
		}
	...
	if (!success)
	{
		...
		for (Index i = 0; i < parameters_number; i++)
		{
			...
			{
				back_propagation_lm.parameters(i) =
					back_propagation_lm.parameters(i);
			...
			else if (back_propagation_lm.gradient(i) > type(0))
			{
				...
				//= nextafter(back_propagation_lm.parameters(i),
				// back_propagation_lm.parameters(i)-1);
...
/// Serializes the Levenberg Marquardt algorithm object into an XML document of
/// the TinyXML library without keeping the DOM tree in memory. See the OpenNN
/// manual for more information about the format of this document.

void LevenbergMarquardtAlgorithm::write_XML(
	tinyxml2::XMLPrinter& file_stream) const
{
	ostringstream buffer;
	...
	const tinyxml2::XMLDocument& document)
{
	const tinyxml2::XMLElement* root_element =
		document.FirstChildElement("LevenbergMarquardt");
	...
	{
		const type new_damping_parameter_factor = static_cast<type>(
	...
	const tinyxml2::XMLElement* maximum_epochs_number_element =
		root_element->FirstChildElement("MaximumEpochsNumber");

	if (maximum_epochs_number_element)
	{
		const Index new_maximum_epochs_number =
			static_cast<Index>(atoi(maximum_epochs_number_element->GetText()));
