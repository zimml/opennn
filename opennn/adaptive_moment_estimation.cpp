...
/// Default constructor.
/// It creates an adaptive moment estimation optimization algorithm not
/// associated with any loss index object. It also initializes the class members
/// to their default values.

AdaptiveMomentEstimation::AdaptiveMomentEstimation() : OptimizationAlgorithm()
{
	set_default();
}

/// Loss index constructor.
/// It creates an adaptive moment estimation optimization algorithm associated
/// with a loss index. It also initializes the class members to their default
/// values.
...
void AdaptiveMomentEstimation::set_batch_samples_number(
...
void AdaptiveMomentEstimation::set_initial_learning_rate(
	const type& new_learning_rate)
{
...
/// Sets a pointer to a loss index object to be associated with the gradient
/// descent object. It also sets that loss index to the learning rate algorithm.
/// @param new_loss_index_pointer Pointer to a loss index object.

void AdaptiveMomentEstimation::set_loss_index_pointer(
	LossIndex* new_loss_index_pointer)
{
...
void AdaptiveMomentEstimation::set_maximum_epochs_number(
	...
	std::function<void(TrainingStep)> callback)
{
	...
	if (display)
		cout << "Training with adaptive moment estimation \"Adam\" ...\n";
	...
	selection_samples_number <
				batch_samples_number&& selection_samples_number !=
			0
	...
	const Index training_batches_number =
		training_samples_number / batch_size_training;
	const Index selection_batches_number =
		selection_samples_number / batch_size_selection;

	Tensor<Index, 2> training_batches(training_batches_number,
									  batch_size_training);
	Tensor<Index, 2> selection_batches(selection_batches_number,
	...
	LossIndexBackPropagation training_back_propagation(batch_size_training,
													   loss_index_pointer);
	LossIndexBackPropagation selection_back_propagation(batch_size_selection,
														loss_index_pointer);
	...
	for (Index epoch = 0; epoch <= maximum_epochs_number; epoch++)
	{
		...
		training_batches =
			data_set_pointer->get_batches(training_samples_indices,
										  batch_size_training,
										  shuffle);
		...
		{
			// Data set
			batch_training.fill(training_batches.chip(iteration, 0),
			...
			// Loss index
			loss_index_pointer->back_propagate(batch_training,
											   training_forward_propagation,
											   training_back_propagation);
			results.training_error_history(epoch) =
				training_back_propagation.error;
		...
		{
			selection_batches =
				data_set_pointer->get_batches(selection_samples_indices,
											  batch_size_selection,
											  shuffle);

			selection_error = type(0);

			for (Index iteration = 0; iteration < selection_batches_number;
				 iteration++)
			{
				// Data set

				batch_selection.fill(selection_batches.chip(iteration, 0),
									 input_variables_indices,
									 target_variables_indices);
				...
				loss_index_pointer->calculate_errors(
					batch_selection,
					selection_forward_propagation,
					selection_back_propagation);

				loss_index_pointer->calculate_error(
					batch_selection,
					selection_forward_propagation,
					selection_back_propagation);
...
void AdaptiveMomentEstimation::update_parameters(
	...
	AdaptiveMomentEstimationData& optimization_data) const
{
	const type learning_rate =
		type(initial_learning_rate *
			 sqrt(type(1) -
				  pow(beta_2, static_cast<type>(optimization_data.iteration))) /
			 (type(1) -
	...
	back_propagation.loss_index_pointer->get_neural_network_pointer()
...
/// Serializes the adaptive moment estimation object into an XML document of the
...
void AdaptiveMomentEstimation::write_XML(
...
/// Imports the adaptive moment estimation object from an XML document of the
/// TinyXML library. See the OpenNN manual for more information about the format
/// of this document.
/// @param document.
...
void AdaptiveMomentEstimation::from_XML(const tinyxml2::XMLDocument& document)
{
	const tinyxml2::XMLElement* root_element =
		document.FirstChildElement("AdaptiveMomentEstimation");
	...
	const tinyxml2::XMLElement* batch_size_element =
		root_element->FirstChildElement("BatchSize");

	if (batch_size_element)
	{
		const Index new_batch_size =
			static_cast<Index>(atoi(batch_size_element->GetText()));
...
/// It creates an adaptive moment estimation data object associated with an
/// adaptive moment estimation algorithm.
/// @param new_adaptive_moment_estimation_pointer Pointer to a adaptive moment
/// estimation object.

AdaptiveMomentEstimationData::AdaptiveMomentEstimationData(
	AdaptiveMomentEstimation* new_adaptive_moment_estimation_pointer)
{
	set(new_adaptive_moment_estimation_pointer);
}

/// Sets a new adaptive moment estimation pointer.
/// @param new_adaptive_moment_estimation_pointer New adaptive moment estimation
/// pointer.

void AdaptiveMomentEstimationData::set(
	AdaptiveMomentEstimation* new_adaptive_moment_estimation_pointer)
{
	adaptive_moment_estimation_pointer = new_adaptive_moment_estimation_pointer;

	LossIndex* loss_index_pointer =
		new_adaptive_moment_estimation_pointer->get_loss_index_pointer();
