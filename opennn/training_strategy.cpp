...
/// Default constructor.
/// It creates a training strategy object not associated with any loss index
/// object. It also constructs the main optimization algorithm object.
...
/// Pointer constuctor.
/// It creates a training strategy object not associated with any loss index
/// object. It also loads the members of this object from NeuralNetwork and
/// DataSet class.

TrainingStrategy::TrainingStrategy(NeuralNetwork* new_neural_network_pointer,
...
TrainingStrategy::get_Levenberg_Marquardt_algorithm_pointer()
{
	return &Levenberg_Marquardt_algorithm;
}
...
TrainingStrategy::get_stochastic_gradient_descent_pointer()
{
	return &stochastic_gradient_descent;
}
...
TrainingStrategy::get_adaptive_moment_estimation_pointer()
{
	return &adaptive_moment_estimation;
}
...
/// Returns the type of the main loss algorithm composing this training strategy
/// object.
...
/// Returns the type of the main optimization algorithm composing this training
/// strategy object.

const TrainingStrategy::OptimizationMethod&
TrainingStrategy::get_optimization_method() const
{
	return optimization_method;
}

/// Returns a string with the type of the main loss algorithm composing this
...
/// Returns a string with the type of the main optimization algorithm composing
/// this training strategy object. If that object does not exists, an exception
/// is thrown.
...
string TrainingStrategy::write_optimization_method() const
{
	...
	else if (optimization_method ==
			 OptimizationMethod::LEVENBERG_MARQUARDT_ALGORITHM)
	{
		return "LEVENBERG_MARQUARDT_ALGORITHM";
	}
	else if (optimization_method ==
			 OptimizationMethod::STOCHASTIC_GRADIENT_DESCENT)
	{
		return "STOCHASTIC_GRADIENT_DESCENT";
	}
	else if (optimization_method ==
			 OptimizationMethod::ADAPTIVE_MOMENT_ESTIMATION)
	{
		return "ADAPTIVE_MOMENT_ESTIMATION";
	}
	else
...
/// false if messages from this class can't be displayed on the screen.
...
void TrainingStrategy::set(NeuralNetwork* new_neural_network_pointer,
						   DataSet* new_data_set_pointer)
{
...
void TrainingStrategy::set_optimization_method(
	const OptimizationMethod& new_optimization_method)
{
	optimization_method = new_optimization_method;
}

/// Sets a new main optimization algorithm from a string containing the type.
/// @param new_optimization_method String with the type of main optimization
/// algorithm.

void TrainingStrategy::set_optimization_method(
	const string& new_optimization_method)
{
...
void TrainingStrategy::set_neural_network_pointer(
...
void TrainingStrategy::set_loss_index_threads_number(
...
void TrainingStrategy::set_optimization_algorithm_threads_number(
	const int& new_threads_number)
{
...
/// Sets a pointer to a loss index object to be associated with the training
/// strategy.
...
void TrainingStrategy::set_loss_index_pointer(LossIndex* new_loss_index_pointer)
{
	...
	Levenberg_Marquardt_algorithm.set_loss_index_pointer(
		new_loss_index_pointer);
}

void TrainingStrategy::set_loss_index_data_set_pointer(
	DataSet* new_data_set_pointer)
{
...
void TrainingStrategy::set_loss_index_neural_network_pointer(
	NeuralNetwork* new_neural_network_pointer)
{
	...
	normalized_squared_error.set_neural_network_pointer(
		new_neural_network_pointer);
	cross_entropy_error.set_neural_network_pointer(new_neural_network_pointer);
	weighted_squared_error.set_neural_network_pointer(
		new_neural_network_pointer);
...
void TrainingStrategy::set_maximum_selection_failures(
	const Index& maximum_selection_failures)
{
	gradient_descent.set_maximum_selection_failures(maximum_selection_failures);
	conjugate_gradient.set_maximum_selection_failures(
		maximum_selection_failures);
	quasi_Newton_method.set_maximum_selection_failures(
		maximum_selection_failures);
	Levenberg_Marquardt_algorithm.set_maximum_selection_failures(
		maximum_selection_failures);
}

void TrainingStrategy::set_maximum_epochs_number(
	const int& maximum_epochs_number)
{
	...
	stochastic_gradient_descent.set_maximum_epochs_number(
	...
	Levenberg_Marquardt_algorithm.set_maximum_epochs_number(
		maximum_epochs_number);
}
	...
	std::function<void(TrainingStep)> callback)
{
	...
	//               << "Convolutional Layer is not available yet. It will be
...
/// If they are not multiples, then the batch size is changed to a multiple (the
/// first multiple that is lower than the batch size).
...
void TrainingStrategy::fix_forecasting()
{
	...
	{
		timesteps = neural_network_pointer->get_recurrent_layer_pointer()
						->get_timesteps();
	}
	else if (neural_network_pointer->has_long_short_term_memory_layer())
	{
		timesteps =
			neural_network_pointer->get_long_short_term_memory_layer_pointer()
				->get_timesteps();
	}
	else
	{
	...
	if (optimization_method == OptimizationMethod::ADAPTIVE_MOMENT_ESTIMATION)
	{
		batch_samples_number =
			adaptive_moment_estimation.get_batch_samples_number();
	}
	else if (optimization_method ==
			 OptimizationMethod::STOCHASTIC_GRADIENT_DESCENT)
	{
		batch_samples_number =
			stochastic_gradient_descent.get_batch_samples_number();
	}
	else
	{
		return;
	...
	else
	{
		const Index constant =
			timesteps > batch_samples_number
				? 1
				: static_cast<Index>(batch_samples_number / timesteps);

		if (optimization_method ==
			OptimizationMethod::ADAPTIVE_MOMENT_ESTIMATION)
		{
			adaptive_moment_estimation.set_batch_samples_number(constant *
																timesteps);
		}
		else if (optimization_method ==
				 OptimizationMethod::STOCHASTIC_GRADIENT_DESCENT)
		{
			stochastic_gradient_descent.set_batch_samples_number(constant *
																 timesteps);
		}
	}
}
...
/// Serializes the training strategy object into an XML document of the TinyXML
/// library without keeping the DOM tree in memory. See the OpenNN manual for
/// more information about the format of this document.
...
void TrainingStrategy::write_XML(tinyxml2::XMLPrinter& file_stream) const
{
	...
	case LossMethod::MEAN_SQUARED_ERROR:
		...
		break;
	case LossMethod::NORMALIZED_SQUARED_ERROR:
		normalized_squared_error.write_regularization_XML(file_stream);
		break;
	case LossMethod::MINKOWSKI_ERROR:
...
void TrainingStrategy::from_XML(const tinyxml2::XMLDocument& document)
{
	...
	const tinyxml2::XMLElement* loss_index_element =
		root_element->FirstChildElement("LossIndex");

	if (loss_index_element)
	{
		const tinyxml2::XMLElement* loss_method_element =
			loss_index_element->FirstChildElement("LossMethod");
		...
		const tinyxml2::XMLElement* Minkowski_error_element =
			loss_index_element->FirstChildElement("MinkowskiError");

		if (Minkowski_error_element)
		{
			tinyxml2::XMLDocument new_document;

			tinyxml2::XMLElement* Minkowski_error_element_copy =
				new_document.NewElement("MinkowskiError");

			for (const tinyxml2::XMLNode* nodeFor =
					 Minkowski_error_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
		...
		const tinyxml2::XMLElement* cross_entropy_element =
			loss_index_element->FirstChildElement("CrossEntropyError");

		if (cross_entropy_element)
		{
			tinyxml2::XMLDocument new_document;

			tinyxml2::XMLElement* cross_entropy_error_element_copy =
				new_document.NewElement("CrossEntropyError");

			for (const tinyxml2::XMLNode* nodeFor =
					 loss_index_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy = nodeFor->DeepClone(&new_document);
		...
		const tinyxml2::XMLElement* weighted_squared_error_element =
			loss_index_element->FirstChildElement("WeightedSquaredError");

		if (weighted_squared_error_element)
		{
			tinyxml2::XMLDocument new_document;

			tinyxml2::XMLElement* weighted_squared_error_element_copy =
				new_document.NewElement("WeightedSquaredError");

			for (const tinyxml2::XMLNode* nodeFor =
					 weighted_squared_error_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy = nodeFor->DeepClone(&new_document);
		...
		const tinyxml2::XMLElement* regularization_element =
			loss_index_element->FirstChildElement("Regularization");
		...
		if (regularization_element)
		{
			...
			element_clone =
				regularization_element->DeepClone(&regularization_document);

			regularization_document.InsertFirstChild(element_clone);

			get_loss_index_pointer()->regularization_from_XML(
				regularization_document);
		}
	}

	// Optimization algorithm

	const tinyxml2::XMLElement* optimization_algorithm_element =
		root_element->FirstChildElement("OptimizationAlgorithm");

	if (optimization_algorithm_element)
	{
		const tinyxml2::XMLElement* optimization_method_element =
			optimization_algorithm_element->FirstChildElement(
				"OptimizationMethod");
		...
		if (gradient_descent_element)
		{
			...
			tinyxml2::XMLElement* gradient_descent_element_copy =
				gradient_descent_document.NewElement("GradientDescent");

			for (const tinyxml2::XMLNode* nodeFor =
					 gradient_descent_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&gradient_descent_document);
				gradient_descent_element_copy->InsertEndChild(copy);
			}

			gradient_descent_document.InsertEndChild(
				gradient_descent_element_copy);
		...
		if (conjugate_gradient_element)
		{
			...
			tinyxml2::XMLElement* conjugate_gradient_element_copy =
				conjugate_gradient_document.NewElement("ConjugateGradient");

			for (const tinyxml2::XMLNode* nodeFor =
					 conjugate_gradient_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&conjugate_gradient_document);
				conjugate_gradient_element_copy->InsertEndChild(copy);
			}

			conjugate_gradient_document.InsertEndChild(
				conjugate_gradient_element_copy);
		...
		const tinyxml2::XMLElement* stochastic_gradient_descent_element =
			optimization_algorithm_element->FirstChildElement(
				"StochasticGradientDescent");
		...
		if (stochastic_gradient_descent_element)
		{
			...
			tinyxml2::XMLElement* stochastic_gradient_descent_element_copy =
				stochastic_gradient_descent_document.NewElement(
					"StochasticGradientDescent");

			for (const tinyxml2::XMLNode* nodeFor =
					 stochastic_gradient_descent_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&stochastic_gradient_descent_document);
				stochastic_gradient_descent_element_copy->InsertEndChild(copy);
			}

			stochastic_gradient_descent_document.InsertEndChild(
				stochastic_gradient_descent_element_copy);

			stochastic_gradient_descent.from_XML(
				stochastic_gradient_descent_document);
		}

		// Adaptive moment estimation

		const tinyxml2::XMLElement* adaptive_moment_estimation_element =
			optimization_algorithm_element->FirstChildElement(
				"AdaptiveMomentEstimation");
		...
		if (adaptive_moment_estimation_element)
		{
			...
			tinyxml2::XMLElement* adaptive_moment_estimation_element_copy =
				adaptive_moment_estimation_document.NewElement(
					"AdaptiveMomentEstimation");

			for (const tinyxml2::XMLNode* nodeFor =
					 adaptive_moment_estimation_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&adaptive_moment_estimation_document);
				adaptive_moment_estimation_element_copy->InsertEndChild(copy);
			}

			adaptive_moment_estimation_document.InsertEndChild(
				adaptive_moment_estimation_element_copy);

			adaptive_moment_estimation.from_XML(
				adaptive_moment_estimation_document);
		}
		...
		if (quasi_Newton_method_element)
		{
			...
			tinyxml2::XMLElement* quasi_newton_method_element_copy =
				quasi_Newton_document.NewElement("QuasiNewtonMethod");

			for (const tinyxml2::XMLNode* nodeFor =
					 quasi_Newton_method_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&quasi_Newton_document);
				quasi_newton_method_element_copy->InsertEndChild(copy);
			}

			quasi_Newton_document.InsertEndChild(
				quasi_newton_method_element_copy);
		...
		const tinyxml2::XMLElement* Levenberg_Marquardt_element =
			optimization_algorithm_element->FirstChildElement(
				"LevenbergMarquardt");
		...
		if (Levenberg_Marquardt_element)
		{
			...
			tinyxml2::XMLElement* levenberg_marquardt_algorithm_element_copy =
				Levenberg_Marquardt_document.NewElement("LevenbergMarquardt");

			for (const tinyxml2::XMLNode* nodeFor =
					 Levenberg_Marquardt_element->FirstChild();
				 nodeFor;
				 nodeFor = nodeFor->NextSibling())
			{
				tinyxml2::XMLNode* copy =
					nodeFor->DeepClone(&Levenberg_Marquardt_document);
				levenberg_marquardt_algorithm_element_copy->InsertEndChild(
					copy);
			}

			Levenberg_Marquardt_document.InsertEndChild(
				levenberg_marquardt_algorithm_element_copy);

			Levenberg_Marquardt_algorithm.from_XML(
				Levenberg_Marquardt_document);
		}
	}
