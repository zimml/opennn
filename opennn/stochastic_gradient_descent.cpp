...
/// Default constructor.
/// It creates a stochastic gradient descent optimization algorithm not
/// associated with any loss index object. It also initializes the class members
/// to their default values.

StochasticGradientDescent::StochasticGradientDescent() : OptimizationAlgorithm()
{
	set_default();
}

/// Loss index constructor.
/// It creates a stochastic gradient descent optimization algorithm associated
...
void StochasticGradientDescent::set_loss_index_pointer(
	LossIndex* new_loss_index_pointer)
{
...
/// Set the initial value for the learning rate. If dacay is not active learning
/// rate will be constant otherwise learning rate will decay over each update.
/// @param new_initial_learning_rate initial learning rate value.

void StochasticGradientDescent::set_initial_learning_rate(
...
/// Set a new value for momentum, this parameter accelerates SGD in the relevant
...
void StochasticGradientDescent::set_maximum_epochs_number(
	...
	StochasticGradientDescentData& optimization_data) const
{
	...
	if (momentum > type(0))
	{
		...
		{
			back_propagation.parameters.device(*thread_pool_device) +=
				optimization_data.parameters_increment * momentum -
				back_propagation.gradient * learning_rate;
			;
		}
	}
	...
	optimization_data.last_parameters_increment =
		optimization_data.parameters_increment;
	...
	std::function<void(TrainingStep)> callback)
{
	...
	if (display)
		cout << "Training with stochastic gradient descent (SGD)...\n";
	...
	for (Index epoch = 0; epoch <= maximum_epochs_number; epoch++)
	{
		...
		if (display && epoch % display_period == 0)
		{
			...
			if (has_selection)
				cout << "Selection error: " << selection_error << endl << endl;
...
Tensor<string, 2> StochasticGradientDescent::to_string_matrix() const
{
	...
	momentum > type(0) ? labels_values(2, 1) = "true"
...
/// Serializes the gradient descent object into an XML document of the TinyXML
...
void StochasticGradientDescent::write_XML(
...
void StochasticGradientDescent::from_XML(const tinyxml2::XMLDocument& document)
{
	const tinyxml2::XMLElement* root_element =
		document.FirstChildElement("StochasticGradientDescent");
	...
	const tinyxml2::XMLElement* apply_momentum_element =
		root_element->FirstChildElement("ApplyMomentum");
