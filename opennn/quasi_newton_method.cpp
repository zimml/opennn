...
/// Default constructor.
/// It creates a quasi-Newton method optimization algorithm not associated with
/// any loss index. It also initializes the class members to their default
/// values.

QuasiNewtonMethod::QuasiNewtonMethod() : OptimizationAlgorithm()
{
	set_default();
}

/// Loss index constructor.
/// It creates a quasi-Newton method optimization algorithm associated with a
/// loss index. It also initializes the class members to their default values.
...
/// Returns a constant reference to the learning rate algorithm object inside
/// the quasi-Newton method object.

const LearningRateAlgorithm& QuasiNewtonMethod::get_learning_rate_algorithm()
	const
{
	return learning_rate_algorithm;
}

/// Returns a pointer to the learning rate algorithm object inside the
...
QuasiNewtonMethod::get_inverse_hessian_approximation_method() const
{
...
/// Returns the maximum number of selection error increases during the training
/// process.
...
/// Sets a pointer to a loss index object to be associated with the quasi-Newton
/// method object. It also sets that loss index to the learning rate algorithm.
/// @param new_loss_index_pointer Pointer to a loss index object.

void QuasiNewtonMethod::set_loss_index_pointer(
...
/// string containing the name. Possible values are: <ul> <li> "DFP" <li> "BFGS"
/// </ul>
...
/// if it is set to false messages from this class are not displayed on the
/// screen.
...
/// @param new_minimum_loss_decrease Minimum improvement in the loss between two
/// epochs.

void QuasiNewtonMethod::set_minimum_loss_decrease(
	const type& new_minimum_loss_decrease)
{
...
void QuasiNewtonMethod::set_maximum_selection_failures(
...
/// @param new_maximum_epochs_number Maximum number of epochs in which the
/// selection evalutation decreases.

void QuasiNewtonMethod::set_maximum_epochs_number(
	const Index& new_maximum_epochs_number)
{
...
void QuasiNewtonMethod::initialize_inverse_hessian_approximation(
	QuasiNewtonMehtodData& optimization_data) const
{
	optimization_data.inverse_hessian.setZero();

	const Index parameters_number =
		optimization_data.inverse_hessian.dimension(0);

	for (Index i = 0; i < parameters_number; i++)
		optimization_data.inverse_hessian(i, i) = type(1);
}
...
/// @param old_inverse_hessian Inverse hessian at the other point of the error
/// function.
	...
	QuasiNewtonMehtodData& optimization_data) const
{
	...
	default:
		...
		buffer
			...
			<< "Tensor<type, 1> calculate_inverse_hessian_approximation(const "
			   "Tensor<type, 1>&, "
			   "const Tensor<type, 1>&, const Tensor<type, 1>&, const "
			   "Tensor<type, 1>&, const Tensor<type, 2>&) method.\n"
			<< "Unknown inverse hessian approximation method.\n";
...
Tensor<type, 2> QuasiNewtonMethod::kronecker_product(
	Tensor<type, 1>& left_matrix, Tensor<type, 1>& right_matrix) const
{
	...
	auto ml = Eigen::Map<
		...
		left_matrix.data(),
		left_matrix.dimension(0),
		1);

	auto mr = Eigen::Map<
		Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
		right_matrix.data(),
		right_matrix.dimension(0),
		1);
	...
	TensorMap<Tensor<type, 2>> direct_matrix(product.data(),
											 left_matrix.size(),
											 left_matrix.size());
...
Tensor<type, 2> QuasiNewtonMethod::kronecker_product(
	Tensor<type, 2>& left_matrix, Tensor<type, 2>& right_matrix) const
{
	// Transform Tensors into Dense matrix
	...
	auto ml = Eigen::Map<
		...
		left_matrix.data(),
		left_matrix.dimension(0),
		left_matrix.dimension(1));

	auto mr = Eigen::Map<
		Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
		right_matrix.data(),
		right_matrix.dimension(0),
		right_matrix.dimension(1));
...
/// Davidon-Fletcher-Powel (DFP) algorithm.
/// @param old_parameters A previous set of parameters.
/// @param old_gradient The gradient of the error function for that previous set
/// of parameters.
/// @param old_inverse_hessian The hessian of the error function for that
/// previous set of parameters.
/// @param parameters Actual set of parameters.
/// @param gradient The gradient of the error function for the actual set of
/// parameters.

void QuasiNewtonMethod::calculate_DFP_inverse_hessian(
	QuasiNewtonMehtodData& optimization_data) const
{
	// Dots
	...
	parameters_difference_dot_gradient_difference.device(*thread_pool_device) =
		optimization_data.parameters_difference.contract(
			optimization_data.gradient_difference,
			AT_B);

	optimization_data.old_inverse_hessian_dot_gradient_difference.device(
		*thread_pool_device) =
		optimization_data.old_inverse_hessian.contract(
			optimization_data.gradient_difference,
			A_B);
	...
	gradient_dot_hessian_dot_gradient.device(*thread_pool_device) =
		optimization_data.gradient_difference.contract(
			optimization_data.old_inverse_hessian_dot_gradient_difference,
			AT_B);	// Ok , auto?
	...
	optimization_data.inverse_hessian +=
		kronecker_product(optimization_data.parameters_difference,
						  optimization_data.parameters_difference) /
	...
	QuasiNewtonMehtodData& optimization_data) const
{
	...
	optimization_data.old_parameters =
		back_propagation.parameters;  // do not move above
	...
	optimization_data.training_direction.device(*thread_pool_device) =
		-optimization_data.inverse_hessian.contract(back_propagation.gradient,
													A_B);

	optimization_data.training_slope.device(*thread_pool_device) =
		back_propagation.gradient.contract(optimization_data.training_direction,
										   AT_B);
	...
	const pair<type, type> directional_point =
		learning_rate_algorithm.calculate_directional_point(batch,
															forward_propagation,
															back_propagation,
															optimization_data);
	...
	if (abs(optimization_data.learning_rate) > type(0))
	{
		optimization_data.parameters_increment.device(*thread_pool_device) =
			...
			optimization_data.learning_rate;
	...
	else
	{
		...
		for (Index i = 0; i < parameters_number; i++)
		{
			...
			else if (back_propagation.gradient(i) > type(0))
			{
				...
				optimization_data.parameters_increment(i) =
					-numeric_limits<type>::epsilon();
			}
			else if (back_propagation.gradient(i) < type(0))
			{
	...
	NeuralNetwork* neural_network_pointer =
		forward_propagation.neural_network_pointer;

	neural_network_pointer->set_parameters(back_propagation.parameters);
}

/// Trains a neural network with an associated loss index according to the
/// quasi-Newton method. Training occurs according to the training operators,
/// training parameters and stopping criteria.

TrainingResults QuasiNewtonMethod::perform_training(
	std::function<void(TrainingStep)> callback)
{
#ifdef OPENNN_DEBUG
	...
	const Index training_samples_number =
		data_set_pointer->get_training_samples_number();

	const Index selection_samples_number =
		data_set_pointer->get_selection_samples_number();
	const bool has_selection = data_set_pointer->has_selection();

	const Tensor<Index, 1> training_samples_indices =
		data_set_pointer->get_training_samples_indices();
	const Tensor<Index, 1> selection_samples_indices =
		data_set_pointer->get_selection_samples_indices();

	const Tensor<Index, 1> input_variables_indices =
		data_set_pointer->get_input_variables_indices();
	const Tensor<Index, 1> target_variables_indices =
		data_set_pointer->get_target_variables_indices();

	const Tensor<string, 1> inputs_names =
		data_set_pointer->get_input_variables_names();
	const Tensor<string, 1> targets_names =
		data_set_pointer->get_target_variables_names();

	const Tensor<Scaler, 1> input_variables_scalers =
		data_set_pointer->get_input_variables_scalers();
	const Tensor<Scaler, 1> target_variables_scalers =
		data_set_pointer->get_target_variables_scalers();
	...
	NeuralNetworkForwardPropagation training_forward_propagation(
		training_samples_number,
		neural_network_pointer);
	NeuralNetworkForwardPropagation selection_forward_propagation(
	...
	{
		input_variables_descriptives =
			data_set_pointer->scale_input_variables();

		ScalingLayer* scaling_layer_pointer =
			neural_network_pointer->get_scaling_layer_pointer();
		scaling_layer_pointer->set(input_variables_descriptives,
								   input_variables_scalers);
	}

	if (neural_network_pointer->has_unscaling_layer())
	{
		target_variables_descriptives =
			data_set_pointer->scale_target_variables();

		UnscalingLayer* unscaling_layer_pointer =
			neural_network_pointer->get_unscaling_layer_pointer();
		unscaling_layer_pointer->set(target_variables_descriptives,
									 target_variables_scalers);
	}

	if (neural_network_pointer->has_flatten_layer())
	{
		FlattenLayer* flatten_layer_pointer =
			neural_network_pointer->get_flatten_layer_pointer();
		// flatten_layer_pointer->set(target_variables_descriptives,
		// target_variables_scalers);
	}

	DataSetBatch training_batch(training_samples_number, data_set_pointer);
	training_batch.fill(training_samples_indices,
						input_variables_indices,
						target_variables_indices);

	DataSetBatch selection_batch(selection_samples_number, data_set_pointer);
	selection_batch.fill(selection_samples_indices,
						 input_variables_indices,
						 target_variables_indices);
	...
	LossIndexBackPropagation training_back_propagation(training_samples_number,
													   loss_index_pointer);
	LossIndexBackPropagation selection_back_propagation(
		selection_samples_number,
		loss_index_pointer);
	...
	for (Index epoch = 0; epoch <= maximum_epochs_number; epoch++)
	{
		...
		if (has_selection)
		{
			neural_network_pointer->forward_propagate(
				selection_batch,
			...
			results.selection_error_history(epoch) =
				selection_back_propagation.error;

			if (epoch != 0 && results.selection_error_history(epoch) >
								  results.selection_error_history(epoch - 1))
				selection_failures++;
		}

		time(&current_time);
		elapsed_time =
			static_cast<type>(difftime(current_time, beginning_time));
		...
		if (display && epoch % display_period == 0)
		{
			cout << "Training error: " << training_back_propagation.error
				 << endl;
			if (has_selection)
				cout << "Selection error: " << selection_back_propagation.error
					 << endl;
		...
		{
			TrainingStep step;
			step.trainingError	= training_back_propagation.error;
			step.selectionError = selection_back_propagation.error;
			step.learningRate	= optimization_data.learning_rate;
		...
		if (epoch != 0)
			loss_decrease = old_loss - training_back_propagation.loss;

		if (loss_decrease < minimum_loss_decrease)
		{
			if (display)
				cout << "Epoch " << epoch << endl
					 << "Minimum loss decrease reached: ("
					 << minimum_loss_decrease << "): " << loss_decrease << endl;

			stop_training = true;

			results.stopping_condition =
				OptimizationAlgorithm::StoppingCondition::MinimumLossDecrease;
		}
		...
		if (results.training_error_history(epoch) < training_loss_goal)
		{
			...
			results.stopping_condition =
				OptimizationAlgorithm::StoppingCondition::LossGoal;

			if (display)
				cout << "Epoch " << epoch << endl
					 << "Loss goal reached: "
					 << results.training_error_history(epoch) << endl;
		}
		else if (selection_failures >= maximum_selection_failures)
		{
			if (display)
				cout << "Epoch " << epoch << endl
					 << "Maximum selection failures reached: "
					 << selection_failures << endl;

			stop_training = true;

			results.stopping_condition = OptimizationAlgorithm::
				StoppingCondition::MaximumSelectionErrorIncreases;
		}
		else if (epoch == maximum_epochs_number)
		{
			...
			results.stopping_condition =
				OptimizationAlgorithm::StoppingCondition::MaximumEpochsNumber;
		}
		else if (elapsed_time >= maximum_time)
		{
			...
			results.stopping_condition =
				OptimizationAlgorithm::StoppingCondition::MaximumTime;
		}
		...
		if (stop_training)
		{
			...
			if (has_selection)
				...
				results.resize_selection_error_history(0);
	...
	if (neural_network_pointer->get_project_type() ==
		NeuralNetwork::ProjectType::AutoAssociation)
	{
		...
		//        neural_network_pointer->calculate_unscaled_outputs(input_data,
		//        inputs_dimensions);
		Tensor<type, 2> outputs =
			neural_network_pointer->calculate_scaled_outputs(input_data,
															 inputs_dimensions);
		...
		Tensor<type, 1> samples_distances =
			neural_network_pointer->calculate_samples_distances(
				input_data,
				inputs_dimensions,
				outputs_data,
				outputs_dimensions);
		...
		BoxPlot distances_box_plot =
			calculate_distances_box_plot(input_data,
										 inputs_dimensions,
										 outputs_data,
										 outputs_dimensions);

		Tensor<type, 2> multivariate_distances =
			neural_network_pointer->calculate_multivariate_distances(
				input_data,
				inputs_dimensions,
				outputs_data,
				outputs_dimensions);
		Tensor<BoxPlot, 1> multivariate_distances_box_plot =
			data_set_pointer->calculate_data_columns_box_plot(
				multivariate_distances);

		neural_network_pointer->set_distances_box_plot(distances_box_plot);
		neural_network_pointer->set_variables_distances_names(
			data_set_pointer->get_input_variables_names());
		neural_network_pointer->set_multivariate_distances_box_plot(
			multivariate_distances_box_plot);
		neural_network_pointer->set_distances_descriptives(
			distances_descriptives);
	}

	data_set_pointer->unscale_input_variables(input_variables_descriptives);

	if (neural_network_pointer->has_unscaling_layer())
		data_set_pointer->unscale_target_variables(
			target_variables_descriptives);
...
/// Serializes the quasi Newton method object into an XML document of the
/// TinyXML library without keeping the DOM tree in memory. See the OpenNN
/// manual for more information about the format of this document.
...
Tensor<string, 2> QuasiNewtonMethod::to_string_matrix() const
{
	...
	labels_values(2, 1) = to_string(
		double(learning_rate_algorithm.get_learning_rate_tolerance()));
...
void QuasiNewtonMethod::from_XML(const tinyxml2::XMLDocument& document)
{
	...
	// Inverse hessian approximation method
	{
		...
		if (element)
		{
			const string new_inverse_hessian_approximation_method =
				element->GetText();
	...
	// Learning rate algorithm
	{
		...
		if (element)
		{
			...
			element_clone =
				element->DeepClone(&learning_rate_algorithm_document);
	...
	{
		const tinyxml2::XMLElement* element =
			root_element->FirstChildElement("MinimumLossDecrease");

		if (element)
		{
			const type new_minimum_loss_decrease =
				static_cast<type>(atof(element->GetText()));
	...
	{
		const tinyxml2::XMLElement* element =
			root_element->FirstChildElement("LossGoal");
	...
	// Maximum selection error increases
	{
		...
		{
			const Index new_maximum_selection_failures =
				static_cast<Index>(atoi(element->GetText()));
	...
	{
		const tinyxml2::XMLElement* element =
			root_element->FirstChildElement("MaximumEpochsNumber");
	...
	{
		const tinyxml2::XMLElement* element =
			root_element->FirstChildElement("HardwareUse");
